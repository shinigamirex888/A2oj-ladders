...
{   
    ...
    for(int i=0;i<n;i++) if (s[i] != '?') {
        p = i; break;   //gives the index of first non ? character
    }//if p=-1 then string only has ?
    if(p==-1){
        for(int i=0;i<n;i++){
            if(i%2){
                cout<<"R";
        ...
        cout<<endl;
        return;
    }//just find the first filled character and fill it's left part and then right part
    for(int i=(p-1);i>=0;i--){
        if(s[i+1]=='B'){
            s[i]='R';
        }else{

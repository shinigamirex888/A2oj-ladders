...
void solution(){
    ll n;
        ...
        if(even%2==0 && odd%2==0){
            if(even != 0){
                // discard two even
                for(ll i=2;i<even_i.size();i=i+2){
                    v.push_back({even_i[i],even_i[i+1]});
                }
                for(ll i=0;i<odd_i.size();i=i+2){
                    v.push_back({odd_i[i],odd_i[i+1]});
                }
            }else{
                // discard two odd
                for(ll i=2;i<odd_i.size();i=i+2){
                    v.push_back({odd_i[i],odd_i[i+1]});
                }
            }
        }else{
            // discard 1 even 1 odd
            for(ll i=1;i<even_i.size();i+=2){
                v.push_back({even_i[i],even_i[i+1]});
            }
            for(ll i=1;i<odd_i.size();i=i+2){
                    v.push_back({odd_i[i],odd_i[i+1]});
                }
        }

...
void solution()
{
    ...
    int start=0,mid=-1;
    for(int i=1;i<n;i++){
        if(v[i]<v[start] && mid==-1)
            start=i;
        else if(v[i]>v[start] && mid==-1)
            mid=i;
        else if(v[i]<v[mid]){
            cout<<"YES"<<endl;
            cout<<start+1<<" "<<mid+1<<" "<<i+1<<endl;
            return;
        }else{
            mid=i;
        }    
...
//finding peak using start=0,mid=-1, compare vi and start twice and mid=-1,compare vi and mid else mid=i